...
function ModuleDashboard({moduleKey}) {
  ...
  return <div style={{color:"var(--chris-text-main)"}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(4,minmax(0,1fr))",gap:16,marginBottom:18}}>{["Records","Pending Actions","Exceptions","This Period"].map((label)=><Metric key={label} label={label}/>)}</div>
    <section style={panelStyle}><SectionHeader title="Operational Areas" subtitle="Functions connected to this module dashboard."/><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(210px,1fr))",gap:12}}>{m.areas.map((area)=><div key={area} style={areaCard}><div style={{fontSize:"var(--chris-font-lg)",fontWeight:800}}>{area}</div><div style={{color:"var(--chris-text-secondary)",fontSize:"var(--chris-font-sm)",lineHeight:1.5,marginTop:8}}>Live analytics and operational shortcuts will activate as this child feature is completed.</div><div style={{color:"var(--chris-gold)",fontSize:"var(--chris-font-xs)",fontWeight:800,marginTop:14}}>PLANNED / CONNECTING</div></div>)}</div></section>
    <section style={{...panelStyle,marginTop:18}}><SectionHeader title="Dashboard Intelligence" subtitle="This dashboard will populate automatically from its child modules as their data becomes available."/><div style={{padding:"18px 0",color:"var(--chris-text-secondary)",fontSize:"var(--chris-font-md)",lineHeight:1.6}}>No analytical data is available yet for this module. CHRIS will replace these placeholders with live KPIs, trends, alerts and recent activity as the underlying functions are completed.</div></section>
  </div>;
}
function Metric({label}) {return <div style={panelStyle}><div style={{color:"var(--chris-text-secondary)",fontSize:"var(--chris-font-sm)",fontWeight:700}}>{label}</div><div style={{fontSize:28,fontWeight:800,marginTop:10}}>—</div><div style={{color:"var(--chris-text-muted)",fontSize:"var(--chris-font-xs)",marginTop:6}}>Activates with module data</div></div>;}
function SectionHeader({title,subtitle}) {return <div style={{marginBottom:18}}><h2 style={{margin:0,fontSize:"var(--chris-font-xl)",fontWeight:800}}>{title}</h2><p style={{margin:"6px 0 0",color:"var(--chris-text-secondary)",fontSize:"var(--chris-font-sm)"}}>{subtitle}</p></div>;}
