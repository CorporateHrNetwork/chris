...
const metricReasons = {
  ...
  FILTERED_SNAPSHOT_HISTORY_UNAVAILABLE: "Historical opening headcount is unavailable for filtered views because existing workforce snapshots are organization-wide.",
  FILTERED_HISTORICAL_CLOSING_UNAVAILABLE: "Historical filtered closing headcount is unavailable because existing workforce snapshots are organization-wide.",
};
...
export default function EmployeeAnalytics() {
  ...
  const workforceScope = useMemo(
    ...
    [filters.departmentId, filters.locationId, filters.status, filters.gender]
  );
  const scopeQuery = useMemo(
    () => new URLSearchParams(
      Object.entries(workforceScope).filter(([, value]) => value)
    ).toString(),
    [workforceScope]
  );
  const scopeActive = Boolean(scopeQuery);
  useEffect(() => {
    let active = true;
    const historyRequest = scopeActive
      ? Promise.resolve({ data: { snapshots: [] } })
      : apiRequest(`/api/analytics/workforce/history?from=${filters.year}-01-01&to=${filters.year}-12-31`);
    Promise.all([
      apiRequest(`/api/analytics/workforce?${query}`),
      historyRequest,
    ]).then(([result, historyResult]) => {
      if (active) {
        setError("");
        setData(result.data);
        setHistory(historyResult.data?.snapshots || []);
      }
    }).catch((err) => active && setError(err.message))
      .finally(() => active && setLoading(false));
    return () => { active = false; };
  }, [query, filters.year, scopeActive]);
  useEffect(() => {
    let active = true;
    ...
    apiRequest(`/api/analytics/workforce/metrics?${params.toString()}`)
      .then((result) => {
        if (active) {
          setMetricsError("");
      ...
      .catch((err) => active && setMetricsError(err.message));
    return () => { active = false; };
  }, [period.from, period.to, workforceScope]);
  ...
  return <main className="wa-page">
    ...
    <section className="wa-card wa-movements"><h2>Workforce Headcount Trend</h2><p>Daily point-in-time current workforce snapshots. No historical values are inferred.</p>{scopeActive ? <div className="wa-empty">Historical headcount trend is unavailable for filtered views because existing snapshots are organization-wide. Clear workforce filters to view the organization trend.</div> : <SnapshotTrend snapshots={history} />}</section>
