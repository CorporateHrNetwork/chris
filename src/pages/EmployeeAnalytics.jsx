...
const metricReasons = {
  INSUFFICIENT_OPENING_SNAPSHOT: "No snapshot exists on or before the period start.",
...
export default function EmployeeAnalytics() {
  ...
  useEffect(() => { let active = true; setLoading(true); setError(""); Promise.all([apiRequest(`/api/analytics/workforce?${query}`), apiRequest(`/api/analytics/workforce/history?from=${filters.year}-01-01&to=${filters.year}-12-31`)]).then(([result, historyResult]) => { if (active) { setData(result.data); setHistory(historyResult.data?.snapshots || []); } }).catch((err) => active && setError(err.message)).finally(() => active && setLoading(false)); return () => { active = false; }; }, [query, filters.year]);
  useEffect(() => { let active = true; setMetricsError(""); apiRequest("/api/analytics/workforce/metrics?from=" + period.from + "&to=" + period.to).then((result) => active && setMetrics(result.data)).catch((err) => active && setMetricsError(err.message)); return () => { active = false; }; }, [period.from, period.to]);
  ...
  return <main className="wa-page">
    ...
    <section className="wa-snapshots"><article><h3>Onboarding</h3><strong>{data.onboarding.averageCompletion}%</strong><span>average completion · {data.onboarding.total} workflows</span></article><article><h3>Leave</h3><strong>{data.leave.approvedCurrentOrUpcoming}</strong><span>approved current or upcoming requests</span></article><article><h3>Attendance today</h3><strong>{data.attendance.recordsToday}</strong><span>{data.attendance.presentToday} present · {data.attendance.lateToday} late · {data.attendance.absentToday} absent</span></article></section>
  </main>;
}
