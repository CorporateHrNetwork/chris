...
test("administrative UI exposes remittance operations and exit financial closure", () => {
  ...
  for (const contract of [
    ...
    "Loan Balance",
    "Salary Advance"
  ]) assert.ok(exits.includes(contract), `exit settlement UI missing ${contract}`);
  for (const endpoint of ["settlement/calculate", "settlement/submit", "settlement/approve", "settlement/waive"]) assert.ok(exitRoutes.includes(endpoint), `exit API missing ${endpoint}`);
