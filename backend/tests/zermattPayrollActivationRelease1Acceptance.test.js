...
test("ZERMATT Release-1 Payroll activation and Nigeria compliance gate", () => {
  ...
  requireText(foundationMigration, [
    'CREATE TABLE "payroll_periods"', 'CREATE TABLE "payroll_salary_rates"', 'CREATE TABLE "payroll_components"',
    'CREATE TABLE "payroll_salary_advances"', 'CREATE TABLE "payroll_runs"', 'CREATE TABLE "payroll_run_lines"',
  ...
  requireText(complianceMigration, [
    'CREATE TABLE "payroll_policy_versions"', 'CREATE TABLE "payroll_tax_reliefs"', 'zermatt-liquor-limited',
    '"basic":57', '"housing":11', '"transport":10', '"meal":9', '"medical":8', '"utility":5',
    '"Full-Time":26', '"Part-Time":16', '["basic","housing","transport"]',
  ...
  requireText(statutory, [
    "calculateAnnualPaye", "calculateStructure", "PAYROLL_STANDARD_DAYS_NOT_CONFIGURED", "PAYROLL_SALARY_RATES_INCOMPLETE",
    "attendancePayrollInput.findMany", "STANDARD_DAYS_DEFAULT", "ATTENDANCE_PAYROLL_INPUT", "pensionableBase",
    "employeePension", "employerPension", "payeTax", "rentReliefAnnual", "nsitfEmployeeDeduction: 0",
    "itfEmployeeDeduction: 0", "CALCULATED_NIGERIA_2026",
  ...
  requireText(approvalCompliance, [
    "validateNigeriaPayrollApproval", "NIGERIA_STATUTORY_IDENTIFIERS_INCOMPLETE", "taxIdentificationNumber",
    "payeState", "pensionPfa", "pensionPin",
  ...
  requireText(routes, [
    ...
    '"/salary-advances"', '"/paid-leave"', '"/runs/draft"', '"/runs/:id/submit"', '"/runs/:id/decision"',
    '"/approvals"', '"/payslips"', 'requirePermission("payroll.view")', 'requirePermission("payroll.process")',
    'requirePermission("payroll.manage")', "PAYROLL_EXECUTION_READINESS_INCOMPLETE", "executeNigeriaDraftPayroll",
  ...
  requireText(readiness, [
    ...
    "readyForExecution: employmentReady && paymentReady && compensationReady", "statutoryCalculationEnabled",
    "paymentTransmissionEnabled: false", "PAYMENT_TRANSMISSION_SEPARATE_CONTROL",
  ], "Payroll readiness");
  ...
  requireText(payrollPage, [
    '"execute"', '"periods"', '"rates"', '"allowances"', '"deductions"', '"payslips"', '"salary-advances"',
    '"paid-leave"', '"approvals"', '"statutory"', '"rent-relief"', "NigeriaPayrollWorkspace",
    "NigeriaPayrollSupplementWorkspace", 'title="Execute Payroll"', 'title="Payroll Periods"', 'title="Salary Rates"',
  ...
  requireText(supplementWorkspace, [
    ...
    "I confirm I reviewed CHRiS-calculated PAYE/pension", 'const path = kind === "ALLOWANCE" ? "allowances" : "deductions"',
  ...
  requireText(payrollSidebar, [
    ...
    '"/payroll?workspace=salary-advances"', '"/payroll?workspace=paid-leave"', '"/payroll?workspace=approvals"', '"/loans"',
  ], "Payroll sidebar");
