...
test("ZERMATT Release-1 Payroll activation and Nigeria compliance gate", () => {
  ...
  assert.equal(complianceMigration.includes('ALTER TABLE "employees"'), false);
  requireText(statutory, ["calculateAnnualPaye","calculateStructure","PAYROLL_STANDARD_DAYS_NOT_CONFIGURED","PAYROLL_SALARY_RATES_INCOMPLETE","attendancePayrollInput.findMany","STANDARD_DAYS_DEFAULT","ATTENDANCE_PAYROLL_INPUT","pensionableBase","employeePension","employerPension","payeTax","rentReliefAnnual","nsitfEmployeeDeduction: 0","itfEmployeeDeduction: 0","CALCULATED_NIGERIA_2026"], "Nigeria payroll calculation service");
  assert.equal(/nsitfEmployeeDeduction:\s*[1-9]/.test(statutory), false);
  assert.equal(/itfEmployeeDeduction:\s*[1-9]/.test(statutory), false);
  requireText(approvalCompliance, ["validateNigeriaPayrollApproval","NIGERIA_STATUTORY_IDENTIFIERS_INCOMPLETE","taxIdentificationNumber","payeState","pensionPfa","pensionPin"], "Nigeria payroll approval compliance");
  requireText(routes, ['"/compliance-policy"','"/tax-reliefs"','"/tax-reliefs/rent"','"/tax-reliefs/:id/decision"','"/periods"','"/salary-rates"','"/salary-rates/template"','"/salary-rates/bulk/preview"','"/salary-rates/bulk/import"','["allowances", "ALLOWANCE"]','["deductions", "DEDUCTION"]','"/salary-advances"','"/paid-leave"','"/runs/draft"','"/runs/:id/submit"','"/runs/:id/decision"','"/approvals"','"/payslips"','requirePermission("payroll.view")','requirePermission("payroll.process")','requirePermission("payroll.manage")',"PAYROLL_EXECUTION_READINESS_INCOMPLETE","executeNigeriaDraftPayroll","validateNigeriaPayrollApproval"], "Payroll routes");
  requireText(operations, ["PAYROLL_PERIOD_OVERLAP","SALARY_RATE_OVERLAP","salaryAdvanceRecoveries","paymentPosted: false",'leaveType: { isPaid: true }'], "Payroll operations");
  assert.equal(/paymentPosted:\s*true/.test(operations), false);
  requireText(readiness, ['FROM "payroll_salary_rates"','FROM "payroll_policy_versions"',"readyForExecution: employmentReady && paymentReady && compensationReady","statutoryCalculationEnabled","paymentTransmissionEnabled: false","PAYMENT_TRANSMISSION_SEPARATE_CONTROL"], "Payroll readiness");
  ...
  requireText(payrollSidebar, ['"/payroll?workspace=execute"','"/payroll?workspace=periods"','"/payroll?workspace=rates"','"/payroll?workspace=allowances"','"/payroll?workspace=deductions"','"/payroll?workspace=payslips"','"/payroll?workspace=salary-advances"','"/payroll?workspace=paid-leave"','"/payroll?workspace=approvals"','"/loans"'], "Payroll sidebar");
  assert.equal(payrollSidebar.includes("planned:"), false);
