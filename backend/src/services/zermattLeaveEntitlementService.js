...
async function ensurePolicyDefinition({ organizationId, actorUserId, definition, tx = prisma }) {
  ...
  const existing = await tx.leavePolicy.findFirst({
    where: { organizationId, code: definition.policyCode, versionNumber: 1 },
  });
  const policyData = {
    organizationId,
  ...
  const policy = existing
    ...
    : await tx.leavePolicy.create({ data: policyData });
  ...
  for (let levelNumber = 1; levelNumber <= 11; levelNumber += 1) {
    ...
    await tx.leaveEntitlementMatrixRule.upsert({
      where: {
        organizationId_levelNumber_leavePolicyId_effectiveFrom: {
          organizationId,
          levelNumber,
      ...
      update: { defaultEntitlement: entitlement, unit: "WORKING_DAYS", newHireTreatment: "FULL", isActive: true },
      create: {
        organizationId,
        levelNumber,
        leavePolicyId: policy.id,
...
async function configureZermattLeavePolicies({ organizationId, actorUserId, tx = prisma }) {
  ...
  for (const definition of POLICY_DEFINITIONS) {
    configured.push(await ensurePolicyDefinition({ organizationId, actorUserId, definition, tx }));
  }
  return configured;
}
    ...
    if (!latest || Number(latest.allocatedEntitlement) !== Number(entitlement) || Number(latest.levelNumber) !== levelNumber) {
      allocation = await tx.leaveEntitlementAllocation.create({
        data: {
          ...
          reason: "ZERMATT Full-Time leave entitlement mapping",
          createdByUserId: actorUserId || null,
        },
      });
    }
