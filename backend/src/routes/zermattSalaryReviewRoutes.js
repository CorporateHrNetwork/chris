...
function dateText(value) { return value ? new Date(value).toISOString().slice(0, 10) : null; }
...
router.post("/payroll/salary-reviews", zermattOnly, requireEmployeeFinancialInputEditor, async (req, res) => {
  try {
    ...
    const activeRates = await prisma.$queryRawUnsafe(
      ...
      req.auth.organizationId, employee.id, effectiveFrom
    );
    ...
    if (current && dateText(current.effectiveFrom) === effectiveFrom) {
      return res.status(409).json({
        status: "error",
        code: "SALARY_REVIEW_EFFECTIVE_DATE_CONFLICT",
        message: "A salary rate already starts on this effective date. Use a later effective date so CHRiS can preserve the existing rate as historical evidence.",
      });
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const created = await payroll.saveSalaryRate({
        organizationId: req.auth.organizationId,
        actorUserId: req.auth.userId,
        input: { employeeNumber, amount, currency: req.body?.currency || current?.currency || "NGN", effectiveFrom, reason: `Individual salary review: ${reason}` },
        prismaClient: tx,
      });
    ...
    return res.status(201).json({ status: "success", message: `${employeeNumber} salary review saved with effective-dated history and audit trail.`, data: { ...result, payrollDraftFreshness: freshness } });
  } catch (error) {
    console.error("Zermatt salary review error:", error);
    return res.status(error.statusCode || 400).json({ status: "error", code: error.code || "SALARY_REVIEW_FAILED", message: error.message || "Unable to apply salary review.", details: error.details });
  }
});
