...
router.get("/", requirePermission("payroll.view"), async (req, res) => {
  try {
    ...
    const salaryRateByEmployee = new Map(salaryRates.map((rate) => [rate.employeeId, rate]));
      ...
      data: employees.map((employee) => {
        ...
        return {
          ...
          currentSalaryRate: currentRate ? {
            ...
            effectiveFrom: currentRate.effectiveFrom ? new Date(currentRate.effectiveFrom).toISOString().slice(0, 10) : null,
            effectiveTo: currentRate.effectiveTo ? new Date(currentRate.effectiveTo).toISOString().slice(0, 10) : null,
    ...
    return res.status(500).json({ status: "error", message: "Unable to load current employees for payroll selection." });
  }
});
