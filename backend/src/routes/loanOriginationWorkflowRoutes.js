...
router.post("/applications", mayApply, async (req, res) => {
  try {
    ...
    const data = await workflow.createDraftApplication({
      organizationId: req.auth.organizationId,
    ...
    return res.status(201).json({
      status: "success",
      message: "Loan application draft created. Attach the completed loan application form before submission for HR verification.",
      data,
    });
  } catch (error) {
...
router.post("/:id/application-form", mayApply, upload.single("file"), async (req, res) => {
  try {
    const data = await workflow.addApplicationForm({
      organizationId: req.auth.organizationId,
      actorUserId: req.auth.userId,
...
router.get("/:id/attachments/:attachmentId/download", mayView, async (req, res) => {
  try {
    const file = await workflow.getAttachment({
      organizationId: req.auth.organizationId,
      loanId: req.params.id,
...
router.post("/:id/submit-for-hr-verification", mayApply, async (req, res) => {
  try {
    const data = await workflow.submitForHrVerification({
      organizationId: req.auth.organizationId,
      actorUserId: req.auth.userId,
      loanId: req.params.id,
...
router.post("/:id/hr-verification", mayVerify, async (req, res) => {
  try {
    const data = await workflow.hrVerificationDecision({
      organizationId: req.auth.organizationId,
      actorUserId: req.auth.userId,
      loanId: req.params.id,
...
router.post("/:id/gm-decision", mayApprove, async (req, res) => {
  try {
    const data = await workflow.gmDecision({
      organizationId: req.auth.organizationId,
      actorUserId: req.auth.userId,
      loanId: req.params.id,
      decision: req.body?.decision,
...
router.post("/:id/disbursement", mayDisburse, async (req, res) => {
  try {
    const data = await workflow.disburseApprovedLoan({
      organizationId: req.auth.organizationId,
      actorUserId: req.auth.userId,
      loanId: req.params.id,
      input: req.body || {},
